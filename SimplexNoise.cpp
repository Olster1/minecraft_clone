...
float SIMPLEX_frequency = 1.0f;
float SIMPLEX_amplitude = 1.0f;
float SIMPLEX_lacunarity = 2.0f;
float SIMPLEX_persistence = 0.5f;
 ...
 */
float SimplexNoise_noise(float x) {
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float SimplexNoise_noise(float x, float y) {
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float SimplexNoise_noise(float x, float y, float z) {
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float SimplexNoise_fractal_1d(size_t octaves, float x, float frequency) {
    float output    = 0.f;
    float denom     = 0.f;
    float amplitude = SIMPLEX_amplitude;

    for (size_t i = 0; i < octaves; i++) {
        output += (amplitude * SimplexNoise_noise(x * frequency));
        denom += amplitude;

        frequency *= SIMPLEX_lacunarity;
        amplitude *= SIMPLEX_persistence;
    }
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float SimplexNoise_fractal_2d(size_t octaves, float x, float y, float frequency) {
    float output = 0.f;
    float denom  = 0.f;
    float amplitude = SIMPLEX_amplitude;

    for (size_t i = 0; i < octaves; i++) {
        output += (amplitude * SimplexNoise_noise(x * frequency, y * frequency));
        denom += amplitude;
 ...
 * @return Noise value in the range[-1; 1], value of 0 on all integer coordinates.
 */
float SimplexNoise_fractal_3d(size_t octaves, float x, float y, float z, float frequency) {
    float output = 0.f;
    float denom  = 0.f;
    float amplitude = SIMPLEX_amplitude;
