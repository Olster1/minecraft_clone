float gui_drawSlider(GameState *gameState, GuiState *guiState, Renderer *renderer, char *id, float value) {
    float width = 20;
    float centerX = 40;
    pushPlainQuadHUD(renderer, make_float3(centerX, 40, 1), make_float2(width, 1), make_float4(1, 1, 1, 1));
    ...
    float3 handleP = make_float3(startX + (value*width), 40, 1);
